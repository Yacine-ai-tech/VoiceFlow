      ...
      <div className="space-y-5">
        ...
        <Card title={<span className="flex items-center gap-2"><Code2 size={16} style={{ color: "var(--accent)" }} /> Reusing the API in your own project</span>}>
          ...
          <ul className="mt-3 list-disc space-y-1.5 pl-5 text-[13px] leading-6 text-dim">
            <li>Every <code className="font-mono text-[12px]">GET</code> (health, analytics, scenarios, benchmarks) skips the token check. <code className="font-mono text-[12px]">POST</code> endpoints and the two WebSocket routes (<code className="font-mono text-[12px]">?token=</code> query param, since a WebSocket handshake can't carry a custom header) are gated behind a shared secret when the deployment sets <code className="font-mono text-[12px]">REQUIRE_INTERNAL_TOKEN=true</code> — off by default. Independent of that flag, every non-static request is also subject to a per-IP rate limit, since not every deployment will want the token gate on.</li>
            ...
            <li>Every heavy compute component (ASR, diarization, TTS) is env-driven local-vs-remote — run it on this host, or point it at a remote endpoint you control (<code className="font-mono text-[12px]">VOICEFLOW_REMOTE_ENDPOINT</code>, <code className="font-mono text-[12px]">VOICEFLOW_TTS_REMOTE_ENDPOINT</code>). Your integration doesn't need to know which one is in effect.</li>
            <li>The webhook relay only fetches destinations that resolve to a public address — private/loopback/link-local targets are rejected — and the agent-tools bridge is a generic contract, not hardcoded to any specific external product. Point either at your own infrastructure or any service that implements the discovery contract in <code className="font-mono text-[12px]">services/agent_tools_bridge.py</code>.</li>
          </ul>
        </Card>
        ...
        <Card title={<span className="flex items-center gap-2"><ShieldCheck size={16} style={{ color: "var(--accent)" }} /> Practical notes</span>}>
          <ul className="list-disc space-y-1.5 pl-5 text-[13px] leading-6 text-dim">
            ...
            <li>A per-IP rate limit applies to every non-static request by default (tune with <code className="font-mono text-[12px]">RATE_LIMIT_HTTP_PER_MIN</code> / <code className="font-mono text-[12px]">RATE_LIMIT_WS_CONNECTS_PER_MIN</code>), and long-running analysis calls are bounded by a timeout (<code className="font-mono text-[12px]">LLM_ANALYSIS_TIMEOUT_SECONDS</code>) rather than left unbounded.</li>
