...
- **External agent tools for the voice agent**: `AGENT_TOOLS_URL` pointing at any service implementing the discovery contract in `services/agent_tools_bridge.py` (unset by default — no tools, voice-only). If that service enforces its own internal-token auth, set `OMNIINTEL_INTERNAL_TOKEN` (or `AGENT_TOOLS_TOKEN` to use a different token specifically for this) to the matching value.
