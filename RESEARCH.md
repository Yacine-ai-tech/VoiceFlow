...
| Action-item extraction, Claude Sonnet 4.6, N=50 real meetings (full run) | P=0.759, R=0.765, F1=0.755 | `ACTION_ITEM_BENCHMARK.md` |
| Action-item extraction, Groq `openai/gpt-oss-120b`, N=8 scored (42 excluded, rate-limited) | P=0.812, R=0.729, F1=0.763 | `ACTION_ITEM_BENCHMARK.md` |
...
The Groq row above is a rerun against the current default
(`openai/gpt-oss-120b`, replacing the earlier, deprecated Llama 3.3 70B
result), but a real Groq daily token-quota ceiling (100,000 TPD) was hit
partway through — 8 of 50 meetings scored before the quota ran out; the
remaining 42 are excluded from the score rather than counted as
extraction failures, since a rate limit and a bad extraction aren't the
same kind of failure. Claude Sonnet has a separate quota and scored the
full 50-meeting run.
